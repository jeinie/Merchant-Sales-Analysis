...
가맹점이 실제로 폐점된 경우와 영업 또는 관리 계약이 종료된 경우는 구분한다. 매출 데이터, AI 분석 이력, 담당자 변경 이력을 보존해야 하므로 가맹점 데이터는 물리 삭제하지 않고 관리 상태를 변경한다.

관리 상태:

- `ACTIVE`: 현재 관리 중
- `CLOSED`: 매장이 실제 폐점됨
- `CONTRACT_ENDED`: 영업 또는 관리 계약 종료로 더 이상 관리 대상이 아님
- `SUSPENDED`: 일시 관리 중단 또는 보류
