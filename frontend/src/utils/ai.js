...
export const generateFranchiseInsight = async (franchise, averages) => {
  ...
  const prompt = `
당신은 "프랜차이즈 본사와 영업 담당자를 위한 지도 기반 가맹점 매출 모니터링 및 운영 인사이트 플랫폼"의 데이터 분석가입니다.
이 서비스의 목적은 점주 컨설팅이 아니라, 본사 관리자와 담당 영업사원이 여러 가맹점 중 어느 매장을 먼저 확인해야 하는지, 어떤 지표를 계속 모니터링해야 하는지 판단하도록 돕는 것입니다.
