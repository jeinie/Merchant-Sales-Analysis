...
지도 기반 가맹점 매출 모니터링과 AI 운영 인사이트를 결합한 본사/영업 담당자용 매출 분석 플랫폼입니다.
...
관리자와 영업사원 계정으로 로그인할 수 있으며, 로그인 후 역할에 따라 접근 가능한 데이터와 관리자 기능이 달라집니다.

![로그인 화면](./docs/screenshots/01-login.png)

### 지도 기반 매출 대시보드

Kakao Maps 위에 가맹점 위치를 표시하고, 최근 월 매출 규모는 마커 크기로, 전월 대비 흐름은 색상으로 구분합니다. 운영 알림 버튼에서는 점검이 필요한 가맹점을 리스트로 확인할 수 있습니다.

![지도 기반 매출 대시보드](./docs/screenshots/02-dashboard-map.png)

### 가맹점 상세 분석 및 AI 인사이트

선택한 가맹점의 월별 매출 추이, 업종 평균 비교, 위험 등급, 점검 사유, Gemini 기반 AI 운영 인사이트를 한 화면에서 확인합니다.

![가맹점 상세 분석](./docs/screenshots/03-details-insight.png)
...
- 점검 필요/주의/정상 관찰 위험 등급 및 우선순위 점수 표시
- 알림 목록 팝오버에서 알림 필터링, 점검 사유, 운영 태그 확인
- AI 인사이트 저장 및 최근 분석 이력 재조회
...
권장 방식은 `backend/.env`를 만든 뒤 로컬 실행 스크립트로 Cloud SQL Auth Proxy와 Spring Boot 실행 설정을 자동화하는 것입니다.
...
FRANCHISE_JWT_SECRET=change-this-local-secret

CLOUD_SQL_INSTANCE=프로젝트ID:리전:인스턴스명
CLOUD_SQL_PROXY_PATH=cloud-sql-proxy.exe
...
```cmd
cd /d "C:\Users\Jang Jung Yoon\aidlc-table-order\backend"
run-gcp-local.cmd
```

이 스크립트는 `backend/.env`를 읽고, `DB_PORT`가 열려 있지 않으면 Cloud SQL Auth Proxy를 먼저 실행한 뒤 Spring Boot를 시작합니다.

수동 실행이 필요하면 Cloud SQL Auth Proxy를 먼저 실행한 뒤, 다른 터미널에서 백엔드를 실행합니다.
...
로컬에서 `run-gcp-local.cmd` 기본값을 사용할 경우:
...
Gemini API는 백엔드에서 호출하므로 `backend/.env`에 아래 값을 설정합니다.
...
- AI 인사이트는 본사/영업 담당자의 운영 모니터링 관점으로 작성되도록 프롬프트를 설계했습니다.
- 운영 알림은 매출 급락, 매출 급등, 거래 건수 감소, 객단가 감소 기준으로 계산합니다.
- AI 인사이트 이력은 `ai_insight_histories` 테이블에 저장되며, 같은 가맹점을 다시 선택해도 최근 분석 내용을 다시 확인할 수 있습니다.
